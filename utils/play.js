      ...
      for (let i = 0; i < song.tracks.length; i++) {
        ...
        if (song.tracks[i].durationMS > 3000000) {
          const embed1 = new MessageEmbed()
              .setColor('#34eb56')
          // eslint-disable-next-line max-len
              .setDescription('In order to meet the needs of smooth music listening and not being overloaded with data, the bot can only play songs under **1 hours** to provide a good experience. Thank you!');
          queue.remove(song.tracks[i]); // remove song from queue
          // eslint-disable-next-line max-len
          interaction.reply({embeds: [embed1]}); // message.reply(`**Song is too long!**`)
        }
        if (song.tracks[i].durationMS === 0) {
          ...
          queue.remove(song.tracks[i]);
          // eslint-disable-next-line max-len
          interaction.reply(`**Removed** ${song.tracks[i].title} | **Reason:** Video stream can't in a playlist`);
        }
      }
      // eslint-disable-next-line max-len, camelcase
